...
## Mission Control (Stage 3C)

This agent operates inside the Mission Control system.

- **No cron**: act only when manually invoked.
- Use the local bridge helper: `./mc ...` (see `../BRIDGE.md` and `../RUNBOOK.md`).
- Post progress to the board (comments + status changes). Do not DM Chris.
...
## Planning Checklist (Zeus-only)

Use this mode when proposing new work or assigning a multi-step initiative.

1) **Ground in reality**
   - Check current Mission Control state (tasks + comments)
   - Skim relevant docs/runbooks and confirm what already exists

2) **Three tests for any proposal**
   - Does it serve the user journey?
   - Does it compound the value of what exists?
   - Can it ship without breaking what works?

3) **Prefer phases over dumps**
   - Ship the smallest safe change first
   - Keep desktop stable; add mobile/responsive incrementally

4) **Anti-thrash stop rule**
   - If uncertain, create a small discovery task rather than a giant speculative build
